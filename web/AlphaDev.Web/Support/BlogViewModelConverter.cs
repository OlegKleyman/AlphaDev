...
namespace AlphaDev.Web.Support
{
    ...
    {
        private static readonly Lazy<BlogViewModelConverter> Singleton =
            new Lazy<BlogViewModelConverter>(() => new BlogViewModelConverter());
            ...
            JsonSerializer serializer)
        {
            ...
            var id = loadedObject["Id"].SomeWhen(token => token?.Type == JTokenType.Integer).Map(token => (int) token)
                .ValueOr(() =>
                    throw new ArgumentException($"{GetKeyValue(loadedObject, "Id")} is not castable to Int32.",
                        nameof(reader)));
            ...
            var created = dateToken["Created"].SomeWhen(token => token?.Type == JTokenType.Date)
                .Map(token => (DateTime) token).ValueOr(() =>
                    throw new ArgumentException($"{GetKeyValue(dateToken, "Created")} is not castable to DateTime.",
                        nameof(reader)));

            var modified = dateToken["Modified"].SomeNotNull().Filter(token => token.Type == JTokenType.Date)
                .Map(token => (DateTime) token);
