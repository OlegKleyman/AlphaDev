...
namespace AlphaDev.Web.Tests.Integration.Features
{
    ...
    public partial class Post_feature : WebFeatureFixture
    {
        ...
        public Post_feature(ITestOutputHelper output, [NotNull] DatabaseWebServerFixture databaseWebServerFixture) :
            base(output,
        ...
        private void When_I_view_it_by_id()
        {
            SiteTester.Posts.GoTo(_postId.ToOption()
                                         .ValueOr(() =>
                                             throw new InvalidOperationException("_postId was not initialized.")));
        }
        ...
        private void Given_there_is_a_blog_post(string title)
        {
            ...
            var blog = DatabasesFixture.BlogContextDatabaseFixture.BlogContext.Blogs.FirstOrDefault()
                                       .SomeNotNull()
        ...
        private CompositeStep When_I_view_a_post_by_id()
        {
            return CompositeStep.DefineNew()
                                .AddSteps(_ => Given_there_is_a_blog_post("testing"), _ => When_I_view_it_by_id())
                                .Build();
        }
        ...
        private void Then_it_should_display_the_title()
        {
            SiteTester.Posts.Post.Title.Should()
                      .BeEquivalentTo(
                          DatabasesFixture.BlogContextDatabaseFixture.BlogContext.Blogs.Find(_postId).Title);
        }

        private void Then_it_should_display_blog_post_with_markdown_parsed_to_html()
        {
            SiteTester.Posts.Post.Content.Should()
                      .BeEquivalentTo(DatabasesFixture.BlogContextDatabaseFixture
                                                      .BlogContext.Blogs
                                                      .OrderByDescending(blog => blog.Created)
                                                      .ToList()
                                                      .Select(blog => Markdown.ToHtml(blog.Content).Trim())
        ...
        {
            SiteTester.Posts.Post.Dates.Modified.Should()
                      .BeEquivalentTo(DatabasesFixture.BlogContextDatabaseFixture
        ...
        {
            SiteTester.Posts.Post.Dates.Created.Should()
                      .BeEquivalentTo(DatabasesFixture.BlogContextDatabaseFixture
                                                      .BlogContext.Blogs.Find(_postId)
                                                      .Created.ToString(FullDateFormatString,
                                                          CultureInfo.InvariantCulture));
        }
        ...
        private void Then_it_should_display_the_posts_menu_link_to_lead_to_all_posts()
        {
            ...
            homePageNavigation.FirstOrNone(element => element is AnchorElement && element.Text == "Posts")
                              .Map(element => new Uri(((AnchorElement) element).Href).AbsoluteUri)
                              .ValueOrFailure()
                              .Should()
                              .BeEquivalentTo(
